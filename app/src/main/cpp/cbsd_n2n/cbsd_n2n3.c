...
JNIEXPORT jobject JNICALL Java_cn_cbsd_vpnx_service_VPNXService_getVPNStatus(
    ...
    if (status.tid != -1) {
        ...
        id = (*env)->GetStaticFieldID(env, clrRunningStatus, "DISCONNECT",
                                      "Lcn/cbsd/vpnx/model/VPNStatus$RunningStatus;");
#ifndef NDEBUG
        __android_log_write(ANDROID_LOG_ERROR, "edge_jni", "status.tid = -1");
#endif /* #ifndef NDEBUG */
    }
...
int ByteToHex(ElemType ch, ElemType bit[8]) {
    int cnt;
    for (cnt = 0; cnt < 8; cnt++) {
//        *(bit+cnt) = (ch>>cnt)&0x01;
...
//    __android_log_print(ANDROID_LOG_DEBUG, "edge_jni", "strBuf = %s",strBuf);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "edge_jni", "js_Enc = %s",js_Enc);
...
//        const char *community = "sxsp";
    ...
    // macAddr
    {
        strncpy(status.cmd.mac_addr, mac, EDGE_CMD_MACNAMSIZ);
#ifndef NDEBUG
        __android_log_print(ANDROID_LOG_DEBUG, "edge_jni", "mac_addr = %s", cmd->mac_addr);
#endif /* #ifndef NDEBUG */
//        char szMac[18];
