...
JNIEXPORT jboolean JNICALL Java_cn_cbsd_vpnx_service_VPNXService_startEdge(
        JNIEnv *env,
        jobject this,
        jobject jcmd) {
...
JNIEXPORT void JNICALL Java_cn_cbsd_vpnx_service_VPNXService_stopEdge(
        JNIEnv *env,
        jobject this) {
...
JNIEXPORT jobject JNICALL Java_cn_cbsd_vpnx_service_VPNXService_getEdgeStatus(
        JNIEnv *env,
        jclass this) {
        ...
        if (!pthread_kill(status.tid, 0)) {
            ...
            switch (status.running_status) {
                ...
                case EDGE_STAT_SUPERNODE_DISCONNECT:
                    id = (*env)->GetStaticFieldID(env, clrRunningStatus, "SUPERNODE_DISCONNECT",
                                                  "Lcn/cbsd/vpnx/model/VPNStatus$RunningStatus;");
                    break;
